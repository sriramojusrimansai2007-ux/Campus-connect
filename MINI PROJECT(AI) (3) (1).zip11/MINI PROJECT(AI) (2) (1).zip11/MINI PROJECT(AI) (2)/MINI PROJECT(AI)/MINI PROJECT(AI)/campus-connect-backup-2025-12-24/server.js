...
app.post('/api/chat', async (req, res) => {
  ...
  // Try OpenAI ChatGPT if API key available
  ...
  if (openaiKey) {
    try {
      ...
      const openaiResponse = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a helpful, knowledgeable AI assistant. Answer any question the user asks - about anything - with accurate, helpful, and concise information. Be friendly and conversational.'
            },
            {
              role: 'user',
              content: message
      ...
      if (openaiResponse.ok) {
        ...
        console.log('OpenAI API error:', openaiResponse.status, errData);
      }
    } catch (e) {
      console.log('OpenAI API unavailable, using fallback:', e.message);
    }
  }
