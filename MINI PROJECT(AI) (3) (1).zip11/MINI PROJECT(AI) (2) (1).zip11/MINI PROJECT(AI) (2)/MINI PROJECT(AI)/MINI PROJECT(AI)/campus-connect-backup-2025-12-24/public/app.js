  ...
  function buildWidget(){
    ...
    async function sendMessage(text){
      ...
      try{
        ...
        if (bots.length) bots[bots.length-1].querySelector('.chat-bubble').textContent = json.reply || 'Sorry, something went wrong.';
      }catch(e){
        const bots = Array.from(body.querySelectorAll('.chat-msg.bot'));
        if (bots.length) bots[bots.length-1].querySelector('.chat-bubble').textContent = 'Failed to reach server.';
      }
    }
